...
class courseViewTest extends TestCase{
    ...
    public function buildDataSet(){
        ...
        (1, 'https://linkt.to/download/a/file.ext', 1, 1, 0),
        ...
        (5, 'http://dowmloadmaterials.com/dl.php?dl=12', 1, 1, 0),
        (6, 'http://dowmloadmaterials.com/dl.php?dl=120', 5, 1, 0),
        (7, 'upload/coursematerials/sem001_2019session.pdf', 5, 0, 0),
        (8, 'upload/coursematerials/sem004_2019secondsemeter.pdf', 1, 0, 0);";
          $db->queryDb($query);
    }

    public function test_fetch_all_course_link_from_database(){
        $result=$this->courseView->fetchAllLinks();
        $result=$this->courseView->getLinksAsArray();
        $this->assertNotEmpty($result,'could not get download links course with id 1');
    }
    public function test_fetch_all_course_link_from_database_as_array(){
        $result=$this->courseView->fetchAllLinks();
        $result=$this->courseView->getLinksAsArray();
        $this->assertIsArray($result,'Expect links as array');
    }
    public function test_fetch_all_course_link_from_database_as_json(){
        $result=$this->courseView->fetchAllLinks();
        $result=$this->courseView->getLinksAsJson();
        $this->assertJson($result,'Expect links as Json');
    }
    public function test_fetch_course_from_database_with_id(){
        $result=$this->courseView->fetchCourseWithId();
        $result=$this->courseView->getCourseAsArray();
        $this->assertNotEmpty($result,'could not get course with id 1');
    }
    public function test_fetch_course_from_database_as_array(){
        $result=$this->courseView->fetchCourseWithId();
        $result=$this->courseView->getLinksAsArray();
        $this->assertIsArray($result,'Expect Course details as array');
    }
    public function test_fetch_course_from_database_as_json(){
        $result=$this->courseView->fetchCourseWithId();
        $result=$this->courseView->getCourseAsJson();
        $this->assertJson($result,'Expect course details as Json');
    }
}